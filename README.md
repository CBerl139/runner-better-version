...
`FoeList` could also be replaced by an `ArrayList<Foe>` attribute inside the `GameScene` class. `InputManaager` is more of a utility class which could (and maybe should) be remove, moving its content to `GameScene` constructor instead.
